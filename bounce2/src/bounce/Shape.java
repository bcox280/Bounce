...
public abstract class Shape {
	...
	final public void paint(Painter painter) {
		// TEMPLATE METHOD
		if (_outputString != null && _outputString.length() > 0) {
			paintString(painter);
		}
		doPaint(painter);
	...
	protected void paintString(Painter painter) {
		
		int x = _x + _width / 2;
		int y = _y + _height / 2;
		painter.drawCentredText(_outputString, x, y);
	...
	public void makeParent(NestingShape shape) {
	...
	public ArrayList<Shape> path() {

		ArrayList<Shape> pathList = new ArrayList<Shape>();
		if (_parent != null) {

			pathList.addAll(_parent.path());
	...
	public void textToOutput(String string) {
